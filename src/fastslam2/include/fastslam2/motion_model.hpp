...
public:
    ...
    std::tuple<double, double, double> sample_motion_model(const std::array<double, 3>& prev_odo, const std::array<double, 3>& curr_odo, const std::array<double, 3>& prev_pose, int move_forward) {

        double rot1 = 0; //assume that rot0 is always zero (pure translation and pure rotation)
        ...
        rot1 = wrapAngle(rot1);
        trans = trans - normalDistributionGen(0, alpha3 * std::pow(trans, 2) + alpha4 * (std::pow(rot1*10, 2) + std::pow(rot2*10, 2)));
    ...
    double motion_model(const std::array<double, 3>& prev_odo, const std::array<double, 3>& curr_odo, const std::array<double, 3>& prev_pose, const std::array<double, 3>& curr_pose, int move_forward) {
        double rot1 = 0;
        rot1 = wrapAngle(rot1);
        double trans = std::sqrt(std::pow(curr_odo[0] - prev_odo[0], 2) + std::pow(curr_odo[1] - prev_odo[1], 2)) * move_forward;
        double rot2 = curr_odo[2] - prev_odo[2] - rot1;
        ...
        double trans_prime = std::sqrt(std::pow(curr_pose[0] - prev_pose[0], 2) + std::pow(curr_pose[1] - prev_pose[1], 2)) * move_forward;
